...
export const getRequests = (key, value) => {
  return (dispatch, getState) => {
    ...
    apiCall(url, state.user, body).then(
...
export const updateRequest = (url, id, comment = null) => {
  return (dispatch, getState) => {
    ...
    apiCall(url, state.user, { id, comment }).then(
...
export const submitComment = (id, comment) => {
  ...
  return dispatch => dispatch(updateRequest("/requests/comment", id, comment));
};

export const approveRequest = id => {
  return dispatch => dispatch(updateRequest("/requests/approve", id));
};

export const rejectRequest = id => {
  return dispatch => dispatch(updateRequest("/requests/reject", id));
};
